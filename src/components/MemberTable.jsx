...
function MemberTable() {
  ...
  useEffect(() => {
    const filtered = memberList.filter((member) => {
      ...
      if (searchCategory === "Name") {
        return name.toLowerCase().includes(searchValue) || lastName.toLowerCase().includes(searchValue);
  ...
  async function addMember(data) {
    ...
    try {
      const requestData = { name: data.name, lastName: data.lastName, complaints: data.complaints };
  ...
  async function saveRow(data) {
    try {
      const requestData = { id: data.id, name: data.name, lastName: data.lastName, complaints: data.complaints };
      const response = await axios.put(`${API_URL}/members/edit`, requestData, {
        headers: { "Content-Type": "application/json", Authorization: sessionStorage.getItem("token") },
      });
  ...
  async function deleteRow(id) {
    ...
    try {
      const response = await axios.delete(`${API_URL}/members/remove/${id}`, {
  ...
  function handleAddMember() { setIsAddMember(true); }
  function handleCancelAddMember() { setIsAddMember(false); }
  function handleBackButton() { navigate("/table"); }
  ...
  return (
    <div className="container">
      ...
      <table className="table">
        ...
        <tbody>
          <RenderTable />
          {isAddMember && <AddMember onCancel={handleCancelAddMember} save={addMember} />}
        </tbody>
      </table>

      <div className="button-container">
        <button className="button memberList" onClick={handleBackButton}>Back</button>
        <button className="button add_member" onClick={handleAddMember}>Add Member</button>
      </div>
